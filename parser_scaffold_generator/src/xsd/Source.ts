...
export class Source {
  ...
  parse(attrTbl: sax.Tag["attributes"] | sax.QualifiedTag["attributes"]) {
    ...
    if (attrTbl["xmlns"]) {
      this.defaultNamespace = this.context.registerNamespace(
        attrTbl["xmlns"].toString()
      );
    }
    ...
    if (attrTbl["targetnamespace"]) {
      if (!this.targetNamespace) {
        this.targetNamespace = this.context.registerNamespace(
          attrTbl["targetnamespace"].toString(),
          this.urlOriginal
        );
        this.targetNamespace.addSource(this);
      }
